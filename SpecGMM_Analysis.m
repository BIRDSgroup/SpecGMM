...
%args.numRepresentations = {"AT_CG", "Atomic", "Codons", "Doublet", "EIIP", "Int", "IntN", "JustA", "JustT", "JustG", "JustC", "PP", "Random3", "Random13", "Real"};
...
for numRep = 1:length(args.numRepresentations)
	...
	for ds = 1:length(dataSets)
		    ...
		    % Construct the file name for the magnitude spectra
			magSpecFileName = strcat(dataSet, '-', string(parameters.winLen), '-', string(parameters.winShift), '-', args.numRepresentations{numRep}, '-MagSpecPooled.mat');
			...
			if args.magSpectra == 1
				...
				% Save the extracted magnitude spectra
				save(magSpecFileName, 'SequenceDataPooled', '-v7.3');

			else
				% Check if the pre-computed magnitude spectra file exists
				if isfile(magSpecFileName)
					fprintf('\nLoading Pre-computed Magnitude Spectra .... \n');
					load(magSpecFileName, 'SequenceDataPooled');
				else
					fprintf('\nPre-computed Magnitude Spectra not found. Computing Magnitude Spectra .... \n');
					% Extract magnitude spectra
					SequenceDataPooled = ComputeMagnitudeSpectra(parameters, data, args.numRepresentations{numRep});

					% Save the newly computed magnitude spectra
					save(magSpecFileName, 'SequenceDataPooled', '-v7.3');
				end
			end
			toc
		    ...
		    if args.analysis16SrRNA == 1
				...
				if args.assignHVR == 1
					...
					save(strcat(dataSet, '-', string(parameters.winLen), '-', string(parameters.winShift), '-', args.numRepresentations{numRep}, '-assignedHVRs_DataPooled.mat'), 'assignedHVRs_DataPooled', '-v7.3');
				else
					fprintf('\nLoading pre-assigned HVRs for windows .... \n');
					% Loading the pre-assigned HVRs
					load(strcat(dataSet, '-', string(parameters.winLen), '-', string(parameters.winShift), '-', args.numRepresentations{numRep}, '-assignedHVRs_DataPooled.mat'), 'assignedHVRs_DataPooled');
				end 
		    end
			...
			for fold = 1:parameters.folds
				...
				% UBM-GMM Training
				ubmFileName = strcat(dataSet, '-', string(parameters.K), '-', string(parameters.winLen), '-', string(parameters.winShift), '-', string(fold), '-', string(args.seedUBM), '-', args.numRepresentations{numRep}, '-UBMGMM.mat');
				...
				if args.trainUBM == 1
					...
					% Save the trained UBM-GMM model
					save(ubmFileName, 'UBMGMM', '-v7.3');

				else
					% Check if the pre-trained UBM-GMM file exists
					if isfile(ubmFileName)
						fprintf('\nLoading Pre-trained UBM-GMM .... \n');
						load(ubmFileName, 'UBMGMM');
					else
						fprintf('\nPre-trained UBM-GMM not found. Training UBM-GMM .... \n');
						tic
							options = statset('MaxIter', 1000);
							UBMGMM = fitgmdist(StackedMagnitudeSpectra, parameters.K, 'CovarianceType', 'diagonal', 'Options', options);
						toc
						
						% Save the newly trained UBM-GMM model
						save(ubmFileName, 'UBMGMM', '-v7.3');
					end
				end
	...
	for ds = 1:length(dataSets)
		metrics = dataSetMetrics.SV.(dataSets{ds});
	...
	% Write to Excel file
	writecell(metricsConsolidated, strcat(args.numRepresentations{numRep}, "-", "ConsolidatedMetrics-SpecGMM.xlsx"));
