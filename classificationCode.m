...
function [accuracy, clNames, sensitivity, specificity, precision, recall, f1_score, predLabels, cMat] = classificationCode(featureVectorsTrain, classLabelsTrain, featureVectorsTest, classLabelsTest, classifiers, seed)
    % Classification using supervised learning on feature vectors

    % Predicted labels for each classifier
    predLabels = cell(length(classifiers), 1);
...
function classifier = trainClassifier(featureVectorsTrain, classLabelsTrain, classifierName)
    ...
    switch classifierName
        ...
        case 'SubspaceDiscriminant'
            subspaceDimension = max(1, min(74, size(featureVectorsTrain, 2) - 1));
